...
var actorChars = {
  "@": Player,
  "o": Baby, // A baby will wobble up and down
  "=": Acid, "|": Acid, "v": Acid,
  "g": Goon,
  "b": Boost
};
...
Goon.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Baby.prototype.act = function(step) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "acid" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "goon" && this.status == null) {
	  this.status = "lost";
	  this.finishDelay = 1;
  } else if (type == "boost") {
    ...
    // If there aren't any babies left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "baby";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
